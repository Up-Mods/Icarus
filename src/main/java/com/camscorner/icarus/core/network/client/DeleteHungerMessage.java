...
public class DeleteHungerMessage {
	...
	public static void handle(MinecraftServer server, ServerPlayerEntity player, ServerPlayNetworkHandler network, PacketByteBuf buf, PacketSender sender) {
		server.execute(() -> {
			player.getHungerManager().addExhaustion(Icarus.config.exhaustionAmount);

			if(Icarus.config.wingsDurability > 0 && player.age % 20 == 0) {
				...
				component.ifPresent(trinketComponent -> trinketComponent.getAllEquipped().forEach(pair -> {
					...
					if(stack.isIn(MELTS))
						stack.damage(1, player.world.random, player);
				}));
			}
		});
	}
